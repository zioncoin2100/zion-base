...
describe('ZionBase#sign', function() {
  ...
  it("can sign an string properly", function() {
    ...
    let actualSig   = ZionBase.sign(data, secretKey).toString('hex');
...
describe('ZionBase#verify', function() {
  ...
  it("can verify an string properly", function() {
    let data = "hello world";
    expect(ZionBase.verify(data, sig, publicKey)).to.be.truthy;
    expect(ZionBase.verify("corrupted", sig, publicKey)).to.be.falsey;
    expect(ZionBase.verify(data, badSig, publicKey)).to.be.falsey;
  });
