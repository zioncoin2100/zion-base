...
describe('Operation', function() {
  describe('.createAccount()', function() {
    it('creates a createAccountOp', function() {
      ...
      let op = ZionBase.Operation.createAccount({
        destination,
        startingBalance
      });
      var xdr = op.toXDR('hex');
      var operation = ZionBase.xdr.Operation.fromXDR(
        Buffer.from(xdr, 'hex')
      );
      var obj = ZionBase.Operation.fromXDRObject(operation);
    ...
    it('fails to create createAccount operation with an invalid destination address', function() {
      ...
      expect(() => ZionBase.Operation.createAccount(opts)).to.throw(
  ...
  describe('.payment()', function() {
    it('creates a paymentOp', function() {
      ...
      var asset = new ZionBase.Asset(
        'USDUSD',
        'GDGU5OAPHNPU5UCLE5RDJHG7PXZFQYWKCFOEXSXNMR6KRQRI5T6XXCD7'
      );
      let op = ZionBase.Operation.payment({ destination, asset, amount });
      var xdr = op.toXDR('hex');
      var operation = ZionBase.xdr.Operation.fromXDR(
        Buffer.from(xdr, 'hex')
      );
    ...
    it('fails to create payment operation with an invalid destination address', function() {
      let opts = {
        destination: 'GCEZW',
        asset: ZionBase.Asset.native(),
        amount: '20'
      };
      expect(() => ZionBase.Operation.payment(opts)).to.throw(
        /destination is invalid/
      );
    });
  ...
  describe('.pathPayment()', function() {
    it('creates a pathPaymentOp', function() {
      var sendAsset = new ZionBase.Asset(
        'USD',
        'GDGU5OAPHNPU5UCLE5RDJHG7PXZFQYWKCFOEXSXNMR6KRQRI5T6XXCD7'
      );
      ...
      var destination =
        'GCEZWKCA5VLDNRLN3RPRJMRZOX3Z6G5CHCGSNFHEYVXM3XOJMDS674JZ';
      var destAsset = new ZionBase.Asset(
        'USD',
        'GDGU5OAPHNPU5UCLE5RDJHG7PXZFQYWKCFOEXSXNMR6KRQRI5T6XXCD7'
      );
      ...
      let op = ZionBase.Operation.pathPayment({
        sendAsset,
        sendMax,
        destination,
        destAsset,
    ...
    it('fails to create path payment operation with an invalid destination address', function() {
      let opts = {
        ...
        sendMax: '20',
        destAmount: '50',
        sendAsset: ZionBase.Asset.native(),
        destAsset: new ZionBase.Asset(
          'USD',
          'GDGU5OAPHNPU5UCLE5RDJHG7PXZFQYWKCFOEXSXNMR6KRQRI5T6XXCD7'
        )
      };
      expect(() => ZionBase.Operation.pathPayment(opts)).to.throw(
        /destination is invalid/
      );
    });
  ...
  describe('.changeTrust()', function() {
    it('creates a changeTrustOp', function() {
      let asset = new ZionBase.Asset(
        'USD',
        'GDGU5OAPHNPU5UCLE5RDJHG7PXZFQYWKCFOEXSXNMR6KRQRI5T6XXCD7'
      );
      let op = ZionBase.Operation.changeTrust({ asset: asset });
      var xdr = op.toXDR('hex');
      var operation = ZionBase.xdr.Operation.fromXDR(
        Buffer.from(xdr, 'hex')
      );
    ...
    it('creates a changeTrustOp with limit', function() {
      ...
      let op = ZionBase.Operation.changeTrust({
        asset: asset,
        limit: '50.0000000'
      });
      var xdr = op.toXDR('hex');
    ...
    it('throws TypeError for incorrect limit argument', function() {
      ...
      let changeTrust = () =>
        ZionBase.Operation.changeTrust({ asset: asset, limit: 0 });
  ...
  describe('.allowTrust()', function() {
    it('creates a allowTrustOp', function() {
      ...
      let assetCode = 'USD';
      let authorize = true;
      let op = ZionBase.Operation.allowTrust({
        trustor: trustor,
        assetCode: assetCode,
        authorize: authorize
      });
    ...
    it('fails to create allowTrust operation with an invalid trustor address', function() {
      ...
      expect(() => ZionBase.Operation.allowTrust(opts)).to.throw(
        /trustor is invalid/
      );
    });
  });
  ...
  describe('.setOptions()', function() {
    it('auth flags are set correctly', function() {
      expect(ZionBase.AuthRequiredFlag).to.be.equal(1);
      expect(ZionBase.AuthRevocableFlag).to.be.equal(2);
      expect(ZionBase.AuthImmutableFlag).to.be.equal(4);
    });
    ...
    it('creates a setOptionsOp', function() {
      ...
      opts.inflationDest =
        'GDGU5OAPHNPU5UCLE5RDJHG7PXZFQYWKCFOEXSXNMR6KRQRI5T6XXCD7';
      opts.clearFlags =
        ZionBase.AuthRevocableFlag | ZionBase.AuthImmutableFlag;
      opts.setFlags = ZionBase.AuthRequiredFlag;
      opts.masterWeight = 0;
      opts.lowThreshold = 1;
      opts.medThreshold = 2;
      ...
      let op = ZionBase.Operation.setOptions(opts);
      var xdr = op.toXDR('hex');
      var operation = ZionBase.xdr.Operation.fromXDR(
        Buffer.from(xdr, 'hex')
      );
    ...
    it('creates a setOptionsOp with preAuthTx signer', function() {
      ...
      var hash = ZionBase.hash('Tx hash');
    ...
    it('creates a setOptionsOp with preAuthTx signer from a hex string', function() {
      ...
      var hash = ZionBase.hash('Tx hash').toString('hex');
    ...
    it('creates a setOptionsOp with hash signer', function() {
      ...
      var hash = ZionBase.hash('Hash Preimage');
    ...
    it('creates a setOptionsOp with hash signer from a hex string', function() {
      ...
      var hash = ZionBase.hash('Hash Preimage').toString('hex');
      expect(isString(hash)).to.be.true;
    ...
    it('empty homeDomain is decoded correctly', function() {
      const keypair = ZionBase.Keypair.random();
      const account = new ZionBase.Account(keypair.publicKey(), '0');

      // First operation do nothing.
      const tx1 = new ZionBase.TransactionBuilder(account, { fee: 100 })
        .addOperation(ZionBase.Operation.setOptions({}))
        .setTimeout(ZionBase.TimeoutInfinite)
        .build();

      // Second operation unset homeDomain
      const tx2 = new ZionBase.TransactionBuilder(account, { fee: 100 })
        .addOperation(ZionBase.Operation.setOptions({ homeDomain: '' }))
        .setTimeout(ZionBase.TimeoutInfinite)
        .build();
    ...
    it('fails to create setOptions operation with an invalid setFlags', function() {
      ...
      expect(() => ZionBase.Operation.setOptions(opts)).to.throw();
    });
  ...
  describe('.manageOffer', function() {
    ...
    it('creates a manageSellOfferOp (string price) (and warns)', function() {
      var opts = {};
      opts.selling = new ZionBase.Asset(
        'USD',
        'GDGU5OAPHNPU5UCLE5RDJHG7PXZFQYWKCFOEXSXNMR6KRQRI5T6XXCD7'
      );
      opts.buying = new ZionBase.Asset(
        'USD',
        'GDGU5OAPHNPU5UCLE5RDJHG7PXZFQYWKCFOEXSXNMR6KRQRI5T6XXCD7'
      );
      ...
      opts.price = '8.141592';
      opts.offerId = '1';
      let op = ZionBase.Operation.manageOffer(opts);
  ...
  describe('.manageSellOffer', function() {
    it('creates a manageSellOfferOp (string price)', function() {
      ...
      opts.price = '8.141592';
      opts.offerId = '1';
      let op = ZionBase.Operation.manageSellOffer(opts);
      var xdr = op.toXDR('hex');
      var operation = ZionBase.xdr.Operation.fromXDR(
        Buffer.from(xdr, 'hex')
      );
    ...
    it('creates an invalid manageSellOfferOp (price fraction)', function() {
      ...
      opts.offerId = '1';
      expect(() => ZionBase.Operation.manageSellOffer(opts)).to.throw(
    ...
    it('fails to create manageSellOffer operation with an invalid amount', function() {
      let opts = {
        amount: 20,
        price: '10',
        selling: new ZionBase.Asset(
          'USD',
          'GDGU5OAPHNPU5UCLE5RDJHG7PXZFQYWKCFOEXSXNMR6KRQRI5T6XXCD7'
        ),
        buying: new ZionBase.Asset(
          'USD',
          'GDGU5OAPHNPU5UCLE5RDJHG7PXZFQYWKCFOEXSXNMR6KRQRI5T6XXCD7'
        )
      };
  ...
  describe('.manageBuyOffer', function() {
    it('creates a manageBuyOfferOp (string price)', function() {
      ...
      opts.price = '8.141592';
      opts.offerId = '1';
      let op = ZionBase.Operation.manageBuyOffer(opts);
      var xdr = op.toXDR('hex');
      var operation = ZionBase.xdr.Operation.fromXDR(
        Buffer.from(xdr, 'hex')
      );
    ...
    it('creates an invalid manageBuyOfferOp (price fraction)', function() {
      ...
      opts.offerId = '1';
      expect(() => ZionBase.Operation.manageBuyOffer(opts)).to.throw(
        /price must be positive/
      );
    });
  ...
  describe('.createPassiveOffer', function() {
    ...
    it('creates a createPassiveSellOfferOp (string price) (and warns)', function() {
      ...
      opts.price = '3.07';
      let op = ZionBase.Operation.createPassiveOffer(opts);
  ...
  describe('.createPassiveSellOffer', function() {
    it('creates a createPassiveSellOfferOp (string price)', function() {
      ...
      opts.amount = '11.2782700';
      opts.price = '3.07';
      let op = ZionBase.Operation.createPassiveSellOffer(opts);
      var xdr = op.toXDR('hex');
      var operation = ZionBase.xdr.Operation.fromXDR(
        Buffer.from(xdr, 'hex')
      );
    ...
    it('fails to create createPassiveSellOffer operation with an invalid amount', function() {
      ...
      expect(() => ZionBase.Operation.createPassiveSellOffer(opts)).to.throw(
        /amount argument must be of type String/
      );
    });
  ...
  describe('.accountMerge', function() {
    it('creates a accountMergeOp', function() {
      ...
      opts.destination =
        'GDGU5OAPHNPU5UCLE5RDJHG7PXZFQYWKCFOEXSXNMR6KRQRI5T6XXCD7';
      let op = ZionBase.Operation.accountMerge(opts);
      var xdr = op.toXDR('hex');
      var operation = ZionBase.xdr.Operation.fromXDR(
        Buffer.from(xdr, 'hex')
      );
    ...
    it('fails to create accountMerge operation with an invalid destination address', function() {
      ...
      expect(() => ZionBase.Operation.accountMerge(opts)).to.throw(
        /destination is invalid/
      );
    });
  });
  ...
  describe('.inflation', function() {
    it('creates a inflationOp', function() {
      let op = ZionBase.Operation.inflation();
      var xdr = op.toXDR('hex');
      var operation = ZionBase.xdr.Operation.fromXDR(
        Buffer.from(xdr, 'hex')
      );
  ...
  describe('.manageData', function() {
    it('creates a manageDataOp with string value', function() {
      ...
      let op = ZionBase.Operation.manageData(opts);
      var xdr = op.toXDR('hex');
      var operation = ZionBase.xdr.Operation.fromXDR(
        Buffer.from(xdr, 'hex')
      );
    ...
    describe('fails to create manageData operation', function() {
      it('name is not a string', function() {
        expect(() =>
          ZionBase.Operation.manageData({ name: 123 })
        ).to.throw();
      });

      it('name is too long', function() {
        expect(() =>
          ZionBase.Operation.manageData({ name: 'a'.repeat(65) })
        ).to.throw();
      });

      it('value is too long', function() {
        expect(() =>
          ZionBase.Operation.manageData({
            name: 'a',
  ...
  describe('.bumpSequence', function() {
    it('creates a bumpSequence', function() {
      ...
      let op = ZionBase.Operation.bumpSequence(opts);
      var xdr = op.toXDR('hex');
      var operation = ZionBase.xdr.Operation.fromXDR(
        Buffer.from(xdr, 'hex')
      );
    ...
    it('fails when `bumpTo` is not string', function() {
      expect(() =>
        ZionBase.Operation.bumpSequence({ bumpTo: 1000 })
      ).to.throw();
    });
  });
  ...
  describe('._checkUnsignedIntValue()', function() {
    it('returns true for valid values', function() {
      ...
      for (var i in values) {
        ...
        expect(
          ZionBase.Operation._checkUnsignedIntValue(value, value)
    ...
    it('return correct values when isValidFunction is set', function() {
      expect(
        ZionBase.Operation._checkUnsignedIntValue(
          'test',
          undefined,
          (value) => value < 10
        )
  ...
  describe('.isValidAmount()', function() {
    it('returns true for valid amounts', function() {
      ...
      for (var i in amounts) {
        expect(ZionBase.Operation.isValidAmount(amounts[i])).to.be.true;
      }
    });
    ...
    it('returns false for invalid amounts', function() {
      ...
      for (var i in amounts) {
        expect(ZionBase.Operation.isValidAmount(amounts[i])).to.be.false;
      }
    });

    it('allows 0 only if allowZero argument is set to true', function() {
      expect(ZionBase.Operation.isValidAmount('0')).to.be.false;
      expect(ZionBase.Operation.isValidAmount('0', true)).to.be.true;
    });
  });

  describe('._fromXDRAmount()', function() {
    it('correctly parses the amount', function() {
      expect(ZionBase.Operation._fromXDRAmount(1)).to.be.equal('0.0000001');
      expect(ZionBase.Operation._fromXDRAmount(10000000)).to.be.equal(
        '1.0000000'
      );
      expect(ZionBase.Operation._fromXDRAmount(10000000000)).to.be.equal(
        '1000.0000000'
      );
      expect(
        ZionBase.Operation._fromXDRAmount(1000000000000000000)
